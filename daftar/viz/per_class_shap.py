...
import logging
from daftar.viz.colors import MULTICLASS_COLORS, BINARY_CLASSIFICATION_COLORS
...
def _per_class_tables(S, feature_names):
    ...
    Returns:
        list: DataFrames, one per class with columns:
            ...
            - mean_magnitude: SHAP magnitude
            - mean_SHAP: SHAP value (direction)
            - rank_by_magnitude: Rank by magnitude
            - rank_by_SHAP: Rank by SHAP value
    """
    tables = []
    for c, s in enumerate(S):  # s → (n_samples, n_features)
        # Compute mean absolute and mean SHAP values
        mean_abs = np.mean(np.abs(s), axis=0)
        mean_shap = np.mean(s, axis=0)
        ...
        # Add ranking columns
        df["rank_by_magnitude"] = df["mean_magnitude"].rank(ascending=False)
        df["rank_by_SHAP"] = df["mean_SHAP"].rank(ascending=False)
        
        # Sort by magnitude and append to result list
        tables.append(df.sort_values("mean_magnitude", ascending=False))
    
    return tables

def _create_summary_plot(shap_values, X, class_id, out_dir):
    """
    Create SHAP summary plot for a specific class.
    
    Args:
        shap_values: SHAP values for the class (n_samples, n_features)
    ...
    # Create SHAP summary plot for this class
    shap.summary_plot(shap_values, X, show=False, plot_type="dot")
    
    plt.title(f'SHAP Summary Plot for Class {class_id}')
    plt.tight_layout()
    
    # Save the plot
    fig = plt.gcf()
    save_plot(fig, os.path.join(out_dir, f'{class_id}_shap_impact.png'), tight_layout=False)
    plt.close()  # Close to prevent memory issues

def _create_barplot(df, class_id, max_display, out_dir):
    """
    Create bar plot showing top positive and negative SHAP features for a class.
    ...
    Args:
        ...
        out_dir: Output directory
    """
    # Sort by SHAP values to get positive and negative features
    pos_features = df.sort_values("mean_SHAP", ascending=False)
    neg_features = df.sort_values("mean_SHAP", ascending=True)
    
    # Use binary classification colors directly
    pos_color = BINARY_CLASSIFICATION_COLORS[0]  # class0 color for positive
    neg_color = BINARY_CLASSIFICATION_COLORS[1]  # class1 color for negative
    ...
    combined = combined.drop_duplicates().sort_values("mean_magnitude", ascending=True)
    ...
    for i, bar in enumerate(bars):
        ...
        bar.set_color(pos_color if value > 0 else neg_color)
    ...
    pos_patch = mpatches.Patch(color=pos_color, label="Positive SHAP")
    neg_patch = mpatches.Patch(color=neg_color, label="Negative SHAP")
    plt.legend(handles=[pos_patch, neg_patch])
    
    # Add labels and title
    plt.title(f'{class_id} SHAP Values\n(Top {max_display} Features by Magnitude)', fontsize=12, pad=15)
    plt.xlabel('SHAP Value', fontsize=10)
    
    # Save the plot
    fig = plt.gcf()
    save_plot(fig, os.path.join(out_dir, f'class_{class_id}_shap_magnitude.png'), tight_layout=False)
...
def report(model, X, *, feature_names=None, max_display=20, out_dir="shap_class_report", 
    ...
    - Creates CSV files with feature magnitude and direction
    ...
    ("Computing per-class SHAP values...")
    S, base = _get_shap_values(model, X)
    n_classes = len(S)
    
    # Generate per-class statistics
    (f"Analyzing SHAP values for {n_classes} classes...")
    ...
    ("Top features per class (by magnitude):")
    for cls_idx, df in enumerate(tables):
        top_features = ", ".join(df.head(5)["feature"].tolist())
        (f"  Class {cls_idx}: {top_features}")
...
def explain_binary_model(model, X, feature_names=None, max_display=20, out_dir="shap_binary_report"):
    ...
    ("For binary classification, focus on Class 1 (positive class) results")
...
def explain_multiclass_model(model, X, feature_names=None, max_display=15, out_dir="shap_multiclass_report"):
    ...
    # Generate class labels for the comparison
    class_labels = [f"Class {i}" for i in range(len(tables))]
    _create_multiclass_comparison(tables, class_labels, max_display, out_dir)
...
def _create_multiclass_comparison(tables, class_labels, max_display, out_dir):
    ...
    Args:
        ...
        out_dir: Output directory
    """
    # Get global feature magnitude by combining across classes
    all_features = set()
    for df in tables:
        all_features.update(df["feature"].tolist())
    
    # Calculate global magnitude
    feature_to_magnitude = {}
    for feature in all_features:
        magnitude = 0
        for df in tables:
            if feature in df["feature"].values:
                magnitude += df.loc[df["feature"] == feature, "mean_magnitude"].values[0]
        feature_to_magnitude[feature] = magnitude
    
    # Sort features by magnitude
    sorted_features = sorted(feature_to_magnitude.items(), 
    ...
    plt.title("SHAP Values Across Classes")
    ...
    fig = plt.gcf()
    save_plot(fig, os.path.join(out_dir, 'multiclass_shap_magnitude_comparison.png'), tight_layout=False)
...
def _standardize_shap_values(shap_vals, n_features):
    ...
    if isinstance(shap_vals, np.ndarray) and shap_vals.ndim == 3:
        ...
        if arr.shape[2] == n_features:
            return arr
        # (n_samples, n_features, n_classes) → moveaxis 2 -> 0  
        elif arr.shape[1] == n_features:
            return np.moveaxis(arr, 2, 0)  # Move classes from last to first
        # (n_samples, n_classes, n_features) → common for new SHAP API
        else:
            return np.moveaxis(arr, 1, 0)  # Move classes from middle to first
...
def save_per_class_shap_analysis(
    ...
    *,
    ...
    Args:
        ...
        config: Configuration object with label encoder for getting original class names.
    ...
    for fr in fold_results:
        ...
        if class_labels is None:
            # First, try to get original class names from label encoder if available
            if (config and hasattr(config, 'label_encoder') and 
                config.label_encoder is not None):
                try:
                    class_labels = list(config.label_encoder.classes_)
                    LOGGER.info(f"Using original class labels from encoder: {class_labels}")
                except Exception as e:
                    LOGGER.warning(f"Could not get class labels from encoder: {e}")
            
            # Fallback: extract from y_test (but these will be encoded values)
            elif y_test is not None:
                try:
                    unique_classes = np.unique(y_test)
                    class_labels = [str(cl) for cl in unique_classes]
                    LOGGER.info(f"Using encoded class labels from y_test: {class_labels}")
                except Exception as e:
    ...
    if class_labels is None or len(class_labels) != all_S_list[0].shape[0]:
        ...
        (f"Using default class labels: {class_labels}")
    ...
    for cls_idx, df in enumerate(tables):
        ...
        consolidated_df[f'{class_label}_Mean_SHAP'] = consolidated_df.index.map(lambda x: class_values.get(x, 0))
    
    # Also add magnitude columns for easy reference
    for cls_idx, df in enumerate(tables):
        class_label = class_labels[cls_idx]
        # Convert to dictionary for easy mapping to features
        class_mag_values = dict(zip(df['feature'], df['mean_magnitude']))
        # Add column with class mean magnitude values
        consolidated_df[f'{class_label}_Mean_Magnitude'] = consolidated_df.index.map(lambda x: class_mag_values.get(x, 0))
    ...
    for cls_idx, df in enumerate(tables):
        ...
        artefacts[f"{class_label}_shap_impact"] = str(out_dir / f"{class_label}_shap_impact.png")
