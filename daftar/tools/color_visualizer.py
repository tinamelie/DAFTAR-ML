...
# Handle imports for both development and installed modes
import os
import sys
from pathlib import Path

def setup_paths():
    """Set up Python path to find modules when running in development mode."""
...
# Try to set up paths before importing
setup_paths()

try:
    # First try the normal import path (for installed package) - use the consolidated colors module
    from daftar.viz.colors import (
        # Binary/multiclass classification colors
        ...
        # Background colors and colormaps
        CORRELATION_CMAP,
        HISTOGRAM_BG_COLOR,
    ...
    print(f"Note: Could not import using package path: {e}")
    print("Attempting direct imports...")
    
    # For development mode, try relative imports
    ...
    print("Using direct imports (development mode)")
...
def main():
    ...
    # Import config path from colors module
    try:
        from daftar.viz.colors import DEFAULT_CONFIG_PATH
    except ImportError:
        # Try relative import for development mode
        sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        from viz.colors import DEFAULT_CONFIG_PATH
    ...
    if args.path:
        ...
        if yaml_path.exists():
            ...
            print(f"DAFTAR-ML colors definitions file is at:\n  {colors_path}")
            print(f"\nBut you should edit the YAML configuration instead at:\n  {yaml_path} (file will be created on first run)")
            print("\nThe YAML configuration takes precedence over hard-coded values.")
        return
