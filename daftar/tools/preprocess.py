...
"""
    ...
    try:
        ...
        if duplicate_ids.any():
            print(f"Warning: Found {duplicate_ids.sum()} duplicate values in the ID column '{id_column}'")
            print("Appending numeric suffixes to make IDs unique...")
            
            # Create a dictionary to track occurrences of each ID
            id_counts = {}
            
            # Function to rename duplicates
            def make_unique_id(id_val):
                if id_val in id_counts:
                    id_counts[id_val] += 1
                    return f"{id_val}_{id_counts[id_val]}"
                else:
                    id_counts[id_val] = 0
                    return id_val
            
            # Apply the function to create unique IDs
            data[id_column] = data[id_column].apply(make_unique_id)
            print(f"ID column '{id_column}' now contains unique values")
