...
def calculate_fold_pvalue(
    train_y: pd.Series,
    test_y: pd.Series,
    is_classification: bool
) -> float:
    """Calculate p-value for fold quality using appropriate statistical test.
    
    For classification tasks, uses chi-square test to compare class distributions.
    For regression tasks, uses Kolmogorov-Smirnov test to compare distributions.
    
    Args:
        train_y: Training set target values
        test_y: Test set target values
        is_classification: Whether the task is classification
        
    Returns:
        p-value indicating statistical similarity between distributions
    """
    if is_classification:
        all_classes = sorted(set(train_y) | set(test_y))
        cont = np.vstack(
            [
                [np.sum(train_y == c) for c in all_classes],
                [np.sum(test_y == c) for c in all_classes],
            ]
        )
        if (cont == 0).any() or len(all_classes) == 1:
            return 1.0
        else:
            return stats.chi2_contingency(cont)[1]
    else:
        return stats.ks_2samp(train_y, test_y)[1]
...
def evaluate_fold_quality(
    ...
    for rep, fold, tr, te in all_splits:
        ...
        pval = calculate_fold_pvalue(tr_y, te_y, is_classification)
...
def _build_base_command(cmd_name: str, args: argparse.Namespace, seed: int, is_classification: bool = None, 
    use_stratification: bool = None, include_default_args: bool = False) -> list[str]:
    """Build a base command list with common parameters.
    
    Args:
        cmd_name: Name of the command (daftar or daftar-cv)
        args: Command line arguments
        seed: Random seed used
        is_classification: Whether the task is classification
        use_stratification: Whether stratification is being used
        include_default_args: Whether to include args even if they're default values
    ...
    if include_default_args or args.inner != 3:
        cmd.append(f"--inner {args.inner}")
    if include_default_args or args.repeats != 3:
        cmd.append(f"--repeats {args.repeats}")
    
    # Add seed parameter
    if args.seed is not None or include_default_args:
        cmd.append(f"--seed {seed if args.seed is None else args.seed}")
    
    # Add stratify flag if needed
    if is_classification is not None and use_stratification is not None:
        ...
        if use_stratification != default_stratification or include_default_args:
...
def build_reproduction_command(args: argparse.Namespace, seed: int, is_classification: bool, use_stratification: bool) -> str:
    ...
    """
    cmd = _build_base_command("daftar-cv", args, seed, is_classification, use_stratification)
    
    # Add additional CV calculator specific parameters
...
def build_daftar_command(args: argparse.Namespace, seed: int, is_classification: bool = None, use_stratification: bool = None) -> str:
    ...
    """
    cmd = _build_base_command("daftar", args, seed, is_classification, use_stratification, True)
    
    # Add DAFTAR specific parameters
    cmd.append("--model [xgb|rf]")
...
def main() -> None:
    ...
    for rep, fold_data in fold_data_by_repeat.items():
        ...
        for fold, tr, te in rep_fold_idxs:
            tr_y, te_y = y.iloc[tr], y.iloc[te]
            pval = calculate_fold_pvalue(tr_y, te_y, is_cls)
    ...
    header_lines = [
        ...
        "-" * 40,
        f"Folds:    {args.outer} outer / {args.inner} inner / {args.repeats} repeats = {args.outer * args.repeats} folds",
        f"Split:    {100 - 100/args.outer:.0f}% train / {100/args.outer:.0f}% test in each fold",
        f"Seed:     {seed}",
    ]
